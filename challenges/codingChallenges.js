...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  const dogsJuliaCorrect = dogsJuliaCopy.splice(1, 2);
  //Part 2:
...
const calcAverageHumanAge = function (ages) {
  //Part 1
  const agesCopy = [...ages];
  const humanAges = agesCopy.map(age => (age <= 2 ? age * 2 : age * 4 + 16));
  ...
  //Part 3
  const finalAverage =
    filteredAges.reduce((acc, age) => acc + age) / filteredAges.length;
...
console.log(
  `Sarah's dog is eating too ${
    sarahDog.curFood > sarahDog.recommendedFood ? 'much' : 'litle'
  }`
);
//3
...
//6
const dogOkay = dogs.some(dog => dog.curFood > (dog.recommendedFood*0.9) && dog.curFood < (dog.recommendedFood *1.1))
console.log(dogOkay);
//7
